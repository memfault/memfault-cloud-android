...
class SampleViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun deviceInfo(serial: String) =
        checkNotNull(deviceInfoMap[serial]) {
            "Illegal selection"
        }

    private fun chunkSender(serial: String) =
        chunkSenderMap.getOrPut(serial) {
            ChunkSender.Builder()
    ...
    fun sendChunks(serial: String) =
        chunkSender(serial).send(
            callback =
                object : SendChunksCallback {
                    ...
                    override fun onRetryAfterDelay(
                        delay: Long,
                        sent: Int,
                        exception: Exception,
